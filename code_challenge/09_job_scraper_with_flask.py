...
from extractors.berlinstartupjobs import extract_berlinstartup_jobs
...
def search():
    ...
    if keyword in db:
        ...
        berlinstartup_jobs = extract_berlinstartup_jobs(keyword)
        weworkremotely_jobs = extract_weworkremotely_jobs(keyword)
        web3_jobs = extract_web3_jobs(keyword)
        jobs = berlinstartup_jobs + weworkremotely_jobs + web3_jobs
        db[keyword] = jobs
    ...
    keywords = db.keys()
    return render_template('search.html', keyword=keyword, jobs=jobs, keywords=keywords)
